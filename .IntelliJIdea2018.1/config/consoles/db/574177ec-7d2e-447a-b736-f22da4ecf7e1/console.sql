...
INSERT INTO topic (decription, name) VALUES
...
INSERT INTO dits.`Question` (description, test_id) VALUES
  ...
  ('Что из нижеперечисленного непроверяемые (unchecked) исключения в джава?', NULL),
  ('Что такое ООП?', NULL),
  ('Что такое класс в Java?', NULL),
  ('Как объявить класс в коде?', NULL),
  ('Для чего используется оператор NEW?', NULL),
  ('Что означает ключевое слово extends?', NULL),
  ('Какое из следующих утверждений верно для класса: java.util.ArrayList?', NULL),
  ('Какое из следующих утверждений верно для method-local inner class?', NULL),
  ('Как указать индекс последнего элемента массива?', NULL),
  ('Что такое --х?', NULL),
...
INSERT INTO dits.`Literature` (`deescription`, `question_id`) VALUES
